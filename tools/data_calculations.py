from tools.data import Nutrition
import csv
import os
...
with open('../data/full/full2019-10-08.csv') as csvfile:
    ...
    for row in readCSV:
        try:
            new_date = Nutrition(row[0], float(row[1]), float(row[2]), float(row[3]), float(row[4]), float(row[5]), float(row[6]), float(row[7]), float(row[8]))
            dates.append(new_date)
        except ValueError:
            print("This is fine, just need to skip first row")


def average_last_30_days(dates_array):
    print("Averages from dates: ")
    print("Beginning: ", dates_array[len(dates_array)-1])
    print("Ending: ", dates_array[len(dates_array) - 31])
    print("---------------------")
    ...
    print("Weight: ", total_weight/30)
    print("Body Fat: ", total_bodyFat/30)
    print("Calories: ", total_calories/30)
    print("Carbs: ", total_carbs/30)
    print("Fat: ", total_fat/30)
    print("Fiber: ", total_fiber/30)
    print("Protein: ", total_protein/30)
    print("Sodium: ", total_sodium/30)
